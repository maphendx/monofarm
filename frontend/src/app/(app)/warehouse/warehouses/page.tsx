    ...
    <div className="space-y-5">
      ...
      <div className="overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--bg-elevated)]">
        {active.map((w) => {
          ...
          const goCells = () => router.push(`/warehouse/warehouses/${w.id}`);
          return (
            <div key={w.id}
              className="group flex items-center gap-3 border-b border-[var(--border)] px-4 py-2.5 last:border-0 hover:bg-[var(--surface-hi)]">
              <button onClick={goCells} className="flex min-w-0 flex-1 items-baseline gap-2 text-left">
                <span className="truncate font-medium group-hover:text-[var(--accent)]">{w.name}</span>
                {w.location && <span className="truncate text-xs text-[var(--text-faint)]">{w.location}</span>}
              </button>

              <span className={`shrink-0 rounded-full px-2 py-0.5 text-xs font-medium ${meta.cls}`}>{meta.label}</span>

              <div className="hidden shrink-0 items-center gap-4 text-xs tabular-nums text-[var(--text-muted)] sm:flex">
                <span><b className="text-[var(--text)]">{stats.sku_count}</b> SKU</span>
                <span><b className="text-[var(--text)]">{Math.round(stats.total_units)}</b> од.</span>
                <span className={stats.zero_stock > 0 ? "text-[var(--state-error)]" : "text-[var(--state-ok)]"}>
                  {stats.zero_stock} нул.
                </span>
              </div>

              <div className="flex shrink-0 items-center gap-1">
                <button onClick={goCells} title="Стелажі / комірки"
                  className="flex size-7 items-center justify-center rounded-md text-[var(--text-faint)] hover:bg-[var(--bg)] hover:text-[var(--text)]">
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                    <rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/>
                    <rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/>
                  </svg>
                </button>
                <button onClick={() => setModalWh(w)} title="Редагувати"
                  className="flex size-7 items-center justify-center rounded-md text-[var(--text-faint)] opacity-0 group-hover:opacity-100 hover:bg-[var(--bg)] hover:text-[var(--text)]">
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
