...
const TYPE_META: Record<MovementType, { label: string; cls: string; needsFrom: boolean; needsTo: boolean }> = {
  ...
  SALE_OUT:       { label: "Продаж",        cls: "bg-cyan-500/15 text-cyan-600 dark:text-cyan-400",          needsFrom: true,  needsTo: false },
  ...
  ADJUSTMENT:     { label: "Коригування",   cls: "bg-neutral-500/15 text-neutral-600 dark:text-neutral-400", needsFrom: false, needsTo: true },
  ...
  return (
    ...
    >
      <form id="movement-form" onSubmit={submit} className="space-y-3 text-sm">
        ...
        {/* Type selector */}
        <div>
          <span className="mb-1.5 block text-neutral-600 dark:text-neutral-400">Тип руху</span>
          <div className="flex flex-wrap gap-1.5">
            {(Object.keys(TYPE_META) as MovementType[]).map((t) => (
              <button key={t} type="button" onClick={() => setMType(t)}
                className={["rounded-md px-2.5 py-1 text-xs font-medium transition-colors",
                  mType === t ? TYPE_META[t].cls + " ring-1 ring-current" : "bg-neutral-100 text-neutral-600 dark:bg-neutral-800 dark:text-neutral-400",
                ].join(" ")}>
                {TYPE_META[t].label}
              </button>
            ))}
        ...
        {(meta.needsFrom || meta.needsTo) && (
          <div className={`grid gap-3 ${meta.needsFrom && meta.needsTo ? "grid-cols-[1fr_auto_1fr]" : "grid-cols-1"}`}>
            {meta.needsFrom && (
              <label className="block">
                <span className="mb-1 block text-neutral-600 dark:text-neutral-400">Звідки</span>
            ...
            {meta.needsFrom && meta.needsTo && (
              <div className="flex items-end pb-2.5 text-neutral-400">→</div>
            )}
            {meta.needsTo && (
              <label className="block">
                <span className="mb-1 block text-neutral-600 dark:text-neutral-400">Куди</span>
        ...
        <div className="grid grid-cols-3 gap-3">
          <label className="block col-span-1">
            <span className="mb-1 block text-neutral-600 dark:text-neutral-400">К-сть</span>
          ...
          <label className="block col-span-1">
            <span className="mb-1 block text-neutral-600 dark:text-neutral-400">Ціна/од.</span>
        ...
        <label className="block">
          <span className="mb-1 block text-neutral-600 dark:text-neutral-400">Причина / коментар</span>
          <input value={reason} onChange={(e) => setReason(e.target.value)} className={inputCls} />
        </label>

        {unitCost && quantity && (
          <div className="rounded-md bg-neutral-50 px-3 py-2 text-xs text-neutral-500 dark:bg-neutral-800">
            Сума: <span className="font-medium text-neutral-700 dark:text-neutral-300">
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex gap-2">
          <button onClick={load} className="rounded-md border border-neutral-200 px-2.5 py-1.5 text-xs hover:bg-neutral-50 dark:border-neutral-800 dark:hover:bg-neutral-800">↻</button>
