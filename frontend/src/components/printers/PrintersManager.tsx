...
function PrinterEditModal({ open, onClose, printer, onDone }: { open: boolean; onClose: () => void; printer: Printer | null; onDone: () => void }) {
  ...
  useEffect(() => {
    if (open) { setForm(printer ? printerToForm(printer) : emptyForm()); setError(null); }
  }, [open, printer]);

  function set<K extends keyof PrinterForm>(k: K, v: PrinterForm[K]) { setForm((f) => ({ ...f, [k]: v })); }
...
export function PrintersManager() {
  ...
  async function loadDiscovered() {
    try { setDiscovered(await api<DiscoveredDevice[]>("/api/printers/bambu/discovered")); }
    catch { setDiscovered([]); }
  }
  ...
  async function handleDelete(id: number) {
    ...
    setDeleteId(id); setConfirmDeleteId(null);
    try {
      await api(`/api/printers/${id}`, { method: "DELETE" });
      setPrinters((ps) => ps.filter((p) => p.id !== id));
    } finally { inFlight.current = false; setDeleteId(null); }
  }
