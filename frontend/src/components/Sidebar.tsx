...
const ICONS: Record<string, string[]> = {
  ...
  setup:     ["M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"],
...
export function Sidebar({ user }: { user: User }) {
  ...
  function toggleLocale() {
    setLocale(locale === "en" ? "uk" : "en" as Locale);
  }

  const NAV = [
    { href: "/dashboard", label: t("nav.dashboard"), icon: "dashboard" },
    { href: "/plan",      label: t("nav.plan"),      icon: "plan" },
    { href: "/files",     label: t("nav.files"),     icon: "files" },
    { href: "/tasks",     label: t("nav.tasks"),     icon: "tasks" },
    { href: "/filament",  label: t("nav.filament"),  icon: "filament" },
    { href: "/analytics", label: t("nav.analytics"), icon: "analytics" },
    { href: "/history",   label: t("nav.history"),   icon: "history" },
    { href: "/printers",  label: t("nav.printers"),  icon: "printers", adminOnly: true },
    { href: "/setup",     label: t("setup.title"),   icon: "setup",    adminOnly: true },
  ...
  return (
    <aside className="group/sidebar fixed inset-y-0 left-0 z-40 flex flex-col border-r border-neutral-200 bg-white transition-[width] duration-200 ease-out w-14 hover:w-[220px] dark:border-neutral-800 dark:bg-neutral-950">
      ...
      <nav className="flex flex-1 flex-col gap-0.5 overflow-hidden px-2 py-3">
        {nav.map((item) => (
          <Link key={item.href} href={item.href} title={item.label} className={linkCls(item.href)}>
      ...
      <div className="flex flex-col gap-0.5 overflow-hidden px-2 py-3">
        ...
        <button
          ...
          className="flex h-9 items-center gap-3 overflow-hidden rounded-lg px-2.5 text-sm text-neutral-500 transition-colors hover:bg-neutral-100 hover:text-neutral-900 dark:text-neutral-400 dark:hover:bg-neutral-800 dark:hover:text-neutral-100"
        >
          <Icon d={ICONS.globe} />
          <span className="whitespace-nowrap opacity-0 transition-opacity duration-150 group-hover/sidebar:opacity-100">
            {locale === "en" ? "UA — Українська" : "EN — English"}
          </span>
        </button>
