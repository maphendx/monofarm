...
**Codex harness:** team config lives in [`.Codex/`](.Codex/AGENTS.md) (`rules/`, `commands/`, `agents/`, shared `settings.json`). Slash workflows: `/backend-test`, `/frontend-check`, `/graphify-refresh`. Optional private notes in `Codex.local.md` (gitignored).
