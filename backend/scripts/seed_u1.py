...
def main(count: int = 12) -> None:
    with SessionLocal() as db:
        ...
        for i in range(1, count + 1):
            name = f"U1-{i:02d}"
            if db.query(Printer).filter(Printer.name == name).first():
                continue
            db.add(Printer(name=name, kind=PrinterKind.snapmaker_u1))
            created += 1
        db.commit()
        print(f"Створено {created} нових U1 принтерів (всього існує {count}).")
