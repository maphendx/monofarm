...
@patch("app.services.bambu_dispatch._region_api_base", return_value="https://api.test")
@patch("app.services.bambu_auth._load_org", return_value=MagicMock(bambu_region="us"))
def test_waits_for_cover_when_profile_is_attached_first(_org, _base):
    pending = {"profiles": [{"profile_id": "42", "context": {"plates": []}}]}
    responses = [_resp(pending), _resp(_PROJECT_DETAIL)]
    with patch.object(bambu_dispatch, "_execute_with_retry", side_effect=responses):
        info = bambu_dispatch.fetch_project_profile(1, "proj_1")
    assert info["profile_id"] == 42
    assert info["cover"] == "https://model-file.bambulab.com/plate_2.png"


@patch("app.services.bambu_dispatch._PROFILE_POLL_DELAY_SECONDS", 0)
@patch("app.services.bambu_dispatch._region_api_base", return_value="https://api.test")
@patch("app.services.bambu_auth._load_org", return_value=MagicMock(bambu_region="us"))
def test_reads_serialized_profile_context(_org, _base):
    detail = {
        "profiles": [{
            "profile_id": "42",
            "context": json.dumps({
                "plates": [{
                    "index": 1,
                    "thumbnail": {"url": "https://model-file.bambulab.com/plate_1.png"},
                }],
            }),
        }],
    }
    ...
    assert info == {
        "profile_id": 42,
        "cover": "https://model-file.bambulab.com/plate_1.png",
        "plate_index": 1,
    }
